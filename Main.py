...
cap = cv2.VideoCapture("C:/Users/84335/PycharmProjects/Main_yolov8/Main3.mp4")
...
while True:
    ...
    results = model(img, stream=True, device='0') # chay tren GPU

    # Ve cac da giac len anh
    for polygon in polygons:
        cv2.polylines(img, [polygon], isClosed=True, color=(0, 0, 255), thickness=2) # Mau do
    ...
    for r in results: # Vong lap hien thi bouding box cho moi ket qua
        ...
        for box in boxes:
            ...
            cv2.rectangle(img, (x1, y1), (x2, y2), (255, 0, 0), 1) #Mau xanh
        ...
        cv2.putText(img, f"FPS: {round(fps, 2)}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

    key = cv2.waitKey(1)
    if key == ord('q'):
        break
    resized_frame = cv2.resize(img, (1000, 640))
